...
async def clearall(ctx):
    await ctx.send("🧹 Starting to clear all messages. This may take a while if there are older messages...")
    
    async for message in ctx.channel.history(limit=None):  # Retrieve all messages
        try:
    ...
    await ctx.send("✅ All messages cleared!", delete_after=5)  # Notify completion
